...
- (void)layoutSubviews {
	...
	CGSize labelSize = [label.text sizeWithFont:label.font];//单行显示根据内容字体大小得到宽和高
	labelSize.width = MIN(labelSize.width, maxWidth);//取最小的值作为宽度
	totalSize.width = MAX(totalSize.width, labelSize.width);
    //根据宽度计算高度用于多行显示
     labelSize = [label.text sizeWithFont:label.font constrainedToSize:CGSizeMake(labelSize.width, MAXFLOAT) lineBreakMode:UILineBreakModeWordWrap];
